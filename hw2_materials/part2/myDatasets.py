...
def get_cifar10_train_val_set(root, ratio=0.9, cv=0):
    ...
    info = np.stack( (np.array(images), np.array(labels)) ,axis=1)#every image-label pair as a row
    N = info.shape[0]
...
class cifar10_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        transform_2_tensor = transforms.Compose([transforms.ToTensor()])

        img = transform_2_tensor(read_img)
        get_labels = self.labels[idx]
        
        
        # You shall return image, label with type "long tensor" if it's training set
        labels = torch.tensor(get_labels, dtype = torch.long)
        if 'unlabeled' in self.prefix:
            return img
        else:
            return img, labels
