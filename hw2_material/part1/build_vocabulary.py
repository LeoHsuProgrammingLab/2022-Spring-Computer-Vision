...
def build_vocabulary(image_paths, vocab_size):
    ...
    for path in tqdm(image_paths):
        ...
        frame, descriptors = dsift(img, step = [step_size, step_size], fast = True)#size = 4
        # print(descriptors.shape)
        for descript in descriptors:
            bag_of_word.append(descript)
        im.close()
        # print(np.array(bag_of_word).shape)
    print('bag_of_word: ', type(bag_of_word))
    print('length = ', len(bag_of_word))
    # before_vocab = time()
    vocab = kmeans(np.matrix(bag_of_word).astype('float32'), vocab_size)
    # after_vocab = time()
    # print(f'The transformation of np.matrix(vocab) = {after_vocab - before_vocab} s')
    print(vocab.shape)
