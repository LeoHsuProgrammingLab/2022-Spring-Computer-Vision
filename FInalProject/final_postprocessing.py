...
def get_avg(data_path, root):
  ...
  for i in range(data_len):
    ...
    area[i] = np.sum(img>128) * 255#adjust
  avg_area = np.sum(area) / data_len
  for i in range(data_len):
    if area[i] < avg_area * 0.6:#adjust
      area[i] = 0
...
def filter(area, avg_area):
  ...
  for i in range(window_len // 2, area_len - window_len // 2):
    ...
    if local_max > 0:
      if(area[i] > local_max * 0.6):#adjust
        result[i] = 1.0
  return result
...
if __name__ == '__main__':
  ...
  for sequence in tqdm(os.listdir(f"{root}")):
    ...
    print(f"{root}/{sequence}/conf.txt")
    result = np.loadtxt(f"{root}/{sequence}/conf.txt")
